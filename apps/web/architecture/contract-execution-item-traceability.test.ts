...
} from "@bba/bdos-core/domain/contract-execution-item-link";
...
const genericMotor = [
  ...
  readFileSync(
    resolve(
      root,
      "packages/bdos-core/src/domain/contract-execution-item-link/contract-execution-item-link.ts",
    ),
    "utf8",
  ),
  readFileSync(
    resolve(
      root,
      "packages/bdos-core/src/domain/contract-execution-item-link/contract-execution-item-link.types.ts",
    ),
    "utf8",
  ),
  readFileSync(
    resolve(
      root,
      "packages/bdos-core/src/services/contract-execution-item-traceability/contract-execution-item-traceability.service.ts",
    ),
    "utf8",
  ),
].join("\n");
...
) as ContractExecutionItemLinkManifest & {
  writeStatus: string;
...
const lagoaValidation = validateContractExecutionItemLinkManifest(manifest);

assert("manifest remains unapplied", manifest.writeStatus === "NOT_APPLIED");
assert("generic engine continues to validate Lagoa", lagoaValidation.valid);
...
assert("generic engine has no hard-coded Lagoa identity", !genericMotor.toLowerCase().includes("lagoa"));
assert("generic engine has no COT-015 rule", !genericMotor.includes("COT-015"));
assert("generic engine has no fixed 300-item rule", !/\b300\b/.test(genericMotor));
assert("generic engine has no fixed 14-exception rule", !/\b14\b/.test(genericMotor));
...
assert("future write is service-role-only and atomic", migration.includes("TO service_role") && migration.includes("v_inserted_count <> v_expected_link_count"));
assert("future write count comes from the approved manifest", migration.includes("v_expected_link_count INTEGER := (p_manifest->'integrity'->>'expectedLinkCount')::INTEGER"));
assert("persisted origin identifies the generic BDOS mechanism", migration.includes("'bdos-contract-execution-item-traceability'"));
...
assert("future write cannot propagate hierarchy or mutate execution and measurements", !migration.includes("UPDATE public.budget_lines") && !migration.includes("UPDATE public.work_packages") && !migration.includes("UPDATE public.managed_service_items") && !migration.includes("measurement_items") && !migration.includes("measurement_entries"));
