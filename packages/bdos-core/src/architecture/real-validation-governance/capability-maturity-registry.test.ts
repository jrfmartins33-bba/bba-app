...
import type { CapabilityMaturityIssue, CapabilityMaturityRecord, CapabilityMaturityRegistry, DownstreamGate, ValidationResult } from "./real-validation-maturity.types";

/**
 * Guarda automatizado da Sprint 21.4G (segunda correção, pós-revisão
 * independente) — valida exclusivamente a ESTRUTURA do registro (o
 * registro estruturado é a fonte atual de maturidade), nunca um scanner
 * textual da prosa de documentação. Bloqueios por finalidade de portão
 * são decididos EXCLUSIVAMENTE por `purposeKind` estruturado e pelo
 * grafo de `dependsOnTargetIds` — nunca por varredura de palavras em
 * `purposePt` (correção de revisão independente).
 *
 ...
 * rejeitada — sem depender do registro real.
 */
...
function collectTransitiveClosure(targetId: string, byId: ReadonlyMap<string, CapabilityMaturityRecord>): ReadonlySet<string> {
...
function detectCycleMembers(byId: ReadonlyMap<string, CapabilityMaturityRecord>): ReadonlySet<string> {
...
export function validateRegistry(registry: CapabilityMaturityRegistry): ReadonlyArray<CapabilityMaturityIssue> {
  ...
  for (const record of registry) {
    ...
    // portões: consumidor/finalidade, purposeKind reconhecido, evidência faltante quando não aberto
    for (const gate of record.downstreamGates) {
      ...
      if (!(GATE_PURPOSE_KINDS as ReadonlyArray<string>).includes(gate.purposeKind)) {
        issues.push({ code: "gate_missing_consumer_or_purpose", recordId: record.id, message: `purposeKind não reconhecido: ${gate.purposeKind}` });
      }
    ...
    // grafo de dependências decide bloqueios de portões real_validation/productive_use — nunca varredura de texto
    if (!cyclicIds.has(record.id)) {
...
function issuesOfCode(code: CapabilityMaturityIssue["code"], registry: CapabilityMaturityRegistry = CAPABILITY_MATURITY_REGISTRY): ReadonlyArray<CapabilityMaturityIssue> {
...
runTest("o registro não está vazio e cobre f.0 a g.3, a caracterização econômica e o cenário ponta a ponta", () => {
  assertTrue(CAPABILITY_MATURITY_REGISTRY.length >= 10, `esperado >= 10 alvos, obteve ${CAPABILITY_MATURITY_REGISTRY.length}`);
});
...
runTest("todo portão possui consumidor, finalidade e purposeKind reconhecido", () => assertEqual(issuesOfCode("gate_missing_consumer_or_purpose").length, 0));
...
runTest("nenhum resultado 'aprovada' esconde uma falha conhecida real", () => assertEqual(issuesOfCode("aprovada_with_known_failure").length, 0));
...
runTest("f.0: nível exercitada_em_caso_real, resultado não avaliada", () => {
  const f0 = CAPABILITY_MATURITY_REGISTRY.find((r) => r.id === "f0-normalized-text-item-geometry");
...
runTest("f.1: nível exercitada_em_caso_real, resultado não avaliada (nunca 'aprovada informal')", () => {
  const f1 = CAPABILITY_MATURITY_REGISTRY.find((r) => r.id === "f1-structure-reconstruction");
...
runTest("f.2a: nível comparada_formalmente_em_caso_real, resultado reprovada, com falhas e evidência adversarial", () => {
  const f2a = CAPABILITY_MATURITY_REGISTRY.find((r) => r.id === "f2a-tabular-region-detection");
  assertTrue(f2a !== undefined, "registro de f.2a deve existir");
  assertEqual(f2a!.currentLevel, "comparada_formalmente_em_caso_real");
...
runTest("caracterização econômica: nível exercitada_em_caso_real, resultado inconclusiva (nunca reprovada) — o defeito pertence a f.2a e ao cenário ponta a ponta", () => {
  const econ = CAPABILITY_MATURITY_REGISTRY.find((r) => r.id === "econ-budget-document-economic-characterization");
...
runTest("cenário ponta a ponta tender-budget-real-extraction-e2e: nível comparada_formalmente_em_caso_real, resultado reprovada, depende de todas as 9 capacidades", () => {
  const e2e = CAPABILITY_MATURITY_REGISTRY.find((r) => r.id === "tender-budget-real-extraction-e2e");
...
runTest("imutabilidade: propriedade de nível do registro não pode ser alterada", () => {
  const record = CAPABILITY_MATURITY_REGISTRY[0];
...
runTest("imutabilidade: status de portão não pode ser alterado", () => {
  const record = CAPABILITY_MATURITY_REGISTRY.find((r) => r.id === "f2a-tabular-region-detection")!;
...
runTest("imutabilidade: o array do registro inteiro não pode receber novo elemento (push)", () => {
  const lengthBefore = CAPABILITY_MATURITY_REGISTRY.length;
  try {
    (CAPABILITY_MATURITY_REGISTRY as unknown as Array<unknown>).push({});
  } catch {
    // esperado
  }
  assertEqual(CAPABILITY_MATURITY_REGISTRY.length, lengthBefore, "o array do registro não pode crescer");
});
...
// inválido (nunca o registro real). Mantidos permanentemente no repositório
// (nunca criados e apagados) — Sprint 21.4G, correção de revisão
// independente, item 9.
// ============================================================================
...
function baseRecord(overrides: Partial<CapabilityMaturityRecord> = {}): CapabilityMaturityRecord {
  return {
    id: "fake-target",
    ...
    promotionConditionPt: null,
    evaluatedRevision: "0000000000000000000000000000000000000",
    ...
    registryVersion: CAPABILITY_MATURITY_REGISTRY[0].registryVersion,
    ...overrides,
  };
}
...
runTest("NEGATIVO — fingerprint truncado é rejeitado", () => {
  const bad = baseRecord({
    ...
    realEvidence: { sourceFingerprintSha256: "5031da75...b92c5", pageOrTraceRange: "x", expectedResult: "x", observedResult: "x", divergences: [], reportReference: "x" },
  });
...
runTest("NEGATIVO — finalidade produtiva aberta com dependência reprovada é rejeitada", () => {
  const upstream = baseRecord({ id: "fake-upstream-reprovada", currentLevel: "comparada_formalmente_em_caso_real", currentResult: "reprovada" });
  const downstream = baseRecord({
    id: "fake-downstream",
    dependsOnTargetIds: ["fake-upstream-reprovada"],
    downstreamGates: [baseGate({ purposeKind: "productive_use", status: "aberto" })],
  });
...
runTest("NEGATIVO — histórico desconectado (encadeamento quebrado) é rejeitado", () => {
  const bad = baseRecord({
    ...
    evaluationHistory: [
      { evaluationId: "e1", date: "2026-01-01", evaluatedRevision: "x", previousLevel: null, previousResult: null, newLevel: "experimental", newResult: "nao_avaliada", evidenceConsideredPt: [], limitationsPt: [], knownFailuresPt: [], implementer: "t", adversarialReviewer: ROLE_NOT_FORMALIZED, approver: ROLE_NOT_FORMALIZED, decisionPt: "d", justificationPt: "j" },
      { evaluationId: "e2", date: "2026-01-02", evaluatedRevision: "x", previousLevel: "evidenciada_sinteticamente", previousResult: "reprovada", newLevel: "evidenciada_sinteticamente", newResult: "aprovada", evidenceConsideredPt: [], limitationsPt: [], knownFailuresPt: [], implementer: "t", adversarialReviewer: ROLE_NOT_FORMALIZED, approver: ROLE_NOT_FORMALIZED, decisionPt: "d", justificationPt: "j" },
    ],
  });
...
runTest("NEGATIVO — alvo reprovado sem falha conhecida é rejeitado", () => {
  const bad = baseRecord({ currentLevel: "comparada_formalmente_em_caso_real", currentResult: "reprovada", knownFailuresPt: [] });
  assertTrue(issuesOfCode("reprovada_without_known_failures", [bad]).length > 0, "guard deve rejeitar reprovada sem falhas conhecidas");
});

runTest("NEGATIVO — alvo inconclusivo sem causa é rejeitado", () => {
  const bad = baseRecord({ currentLevel: "exercitada_em_caso_real", currentResult: "inconclusiva", inconclusiveCausePt: null });
...
runTest("NEGATIVO — mutação de estrutura congelada (registro construído com deepFreeze) não tem efeito", () => {
  ...
  const record = CAPABILITY_MATURITY_REGISTRY.find((r) => r.id === "econ-budget-document-economic-characterization")!;
  attemptMutationHasNoEffect(
    record,
    () => {
