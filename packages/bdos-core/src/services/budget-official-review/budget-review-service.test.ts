...
import { ProcurementScopeKind, createProcurementCase } from "../../domain/procurement-case";
import type { ProcurementCase, ProcurementScope } from "../../domain/procurement-case";
...
function freshBudgetVersion(id: string): BudgetVersion {
...
async function buildReadySession(reviewRepository: BudgetReviewRepository, budgetVersionRepository: BudgetVersionRepository, id: string) {
  const budgetVersion = freshBudgetVersion(`budget-version-${id}`);
  ...
  const confirmedGroup = await confirmBudgetReviewRowService(contextFor(), { sessionId: created.session.id, rowId: "group-1" }, reviewRepository);
  if (confirmedGroup.outcome !== "success") throw new Error("expected group confirmation success");
  const confirmedItem = await confirmBudgetReviewRowService(contextFor(), { sessionId: created.session.id, rowId: "item-confirmed" }, reviewRepository);
  ...
  const excluded = await excludeBudgetReviewRowService(
    contextFor(),
    { sessionId: created.session.id, rowId: "item-excluded", justification: "Não é item real — cabeçalho de seção." },
    reviewRepository,
  );
  if (excluded.outcome !== "success") throw new Error("expected exclusion success");

  return { sessionId: created.session.id, budgetVersionId: budgetVersion.id };
}
...
await runTest("Consolidação projeta apenas linhas aprovadas (ignora Excluída) e persiste BudgetVersion Consolidated", async () => {
  ...
  const result = await consolidateBudgetReviewSessionService(contextFor(), { sessionId }, reviewRepository, budgetVersionRepository);
...
await runTest("Consolidação é idempotente — repetir não duplica BudgetVersion nem regrava a Sessão", async () => {
  ...
  const first = await consolidateBudgetReviewSessionService(contextFor(), { sessionId }, reviewRepository, budgetVersionRepository);
  ...
  const second = await consolidateBudgetReviewSessionService(contextFor(), { sessionId }, reviewRepository, budgetVersionRepository);
