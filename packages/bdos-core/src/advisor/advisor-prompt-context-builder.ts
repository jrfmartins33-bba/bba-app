...
import { deriveDecisionTemporalFacts, deriveRecommendationTemporalFacts } from "./advisor-temporal-facts";
...
// calculados a partir de EngineeringAdvisorHistoricalFacts — dado cru já
// buscado pelo app layer (snapshot anterior + contagem de imports em
// aberto por recommendation_ref_id). Nenhuma composição de dois
...
//
// Sprint 14.4 (Explainability): o cálculo de isNew/previousPriority/
// priorityChanged/openSinceImportCount/recurring foi extraído para
// advisor-temporal-facts.ts — buildEngineeringAdvisorExplanations()
// (Sprint 14.4) reusa exatamente as mesmas funções, para nunca haver
...
export function buildEngineeringAdvisorPromptContext(
  ...
  const recommendations: EngineeringAdvisorPromptRecommendation[] = topRecommendations.map((recommendation) => {
    ...
    return {
      ...
      openSinceImportCount: temporalFacts.openSinceImportCount,
  ...
  const decisions: EngineeringAdvisorPromptDecision[] = relatedDecisions.map((decision) => {
    const temporalFacts = deriveDecisionTemporalFacts(decision, historicalFacts);
    ...
    return {
      ...
      isNew: temporalFacts.isNew,
      previousPriority: temporalFacts.previousPriority,
      priorityChanged: temporalFacts.priorityChanged
    };
  });
