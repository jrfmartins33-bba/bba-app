...
`vertical-alignment-observation.ts` permite no máximo um segmento por linha em cada cluster de alinhamento (§8) — quando uma linha tem múltiplos segmentos que seriam individualmente compatíveis com o mesmo cluster, a atribuição é canônica e determinística (ordem de posição, depois `lineVerticalOrder`, depois `horizontalOrder`, depois `segmentKey`), nunca ambígua ou dependente de ordem de descoberta. Isso é correto e necessário para determinismo, mas é conservador: em páginas com múltiplos segmentos concorrentes na mesma linha disputando a mesma posição de alinhamento, apenas um deles participa do cluster — o(s) outro(s) permanece(m) fora daquele alinhamento específico, mesmo estando geometricamente dentro da tolerância. Isso pode, em tese, reduzir o número de alinhamentos recorrentes observados (nunca aumentá-lo ou produzi-los incorretamente) e, no limite, impedir a formação de uma região que exigiria contar o mesmo segmento em mais de um cluster simultaneamente. **Este é um comportamento conservador esperado, não uma falha desta Sprint** — a alternativa (permitir múltiplas participações do mesmo segmento) introduziria ambiguidade de contagem incompatível com "cada linha sustenta um alinhamento no máximo uma vez" (§8) e não foi aprovada pelo enunciado. O algoritmo não foi alterado nesta revisão; fica registrado como candidato a revisão em Sprint futura com corpus real, quando houver evidência concreta de que o cenário ocorre e de qual seria o comportamento correto.
