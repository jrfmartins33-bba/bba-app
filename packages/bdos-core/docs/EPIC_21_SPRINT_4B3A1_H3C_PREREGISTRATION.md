...
## 17. Desvio metodológico pós-pré-registro — H3c × H3c-r1 (CORREÇÃO, terceiro commit desta Sprint)

**CORREÇÃO OBRIGATÓRIA (revisão humana de Ricardo)**: o texto original desta seção (preservado abaixo, nunca apagado, por auditabilidade) classificava a mudança de identidade de agrupamento (`lineKey` → `segmentKey`) como "correção de implementação divergente da especificação congelada". **Essa classificação estava incorreta e é substituída pela análise abaixo.** A mudança altera a formação dos grupos, os envelopes gerados e, potencialmente, a decisão final — não é um ajuste de representação interna neutro; é uma **mudança semântica da candidata**, feita **depois** da observação de que a definição literalmente congelada reprovava nos testes sintéticos direcionados (P1, P5-P8). O enunciado da Sprint 21.4B.3A.1 §3 proíbe justamente isso sem invalidar a candidata e exigir novo identificador — o que se aplica aqui.

### H3c (original, literalmente pré-registrada em §6.4)

- Identidade de agrupamento: `alignmentType + ":" + lineKeys-dos-membros-ordenados-e-concatenados` — texto de §6.4 acima, nunca alterado retroativamente.
- Sob esta definição literal, H3c **falhou** nos testes sintéticos direcionados (P1, P5-P8): em qualquer tabela densa convencional, todas as colunas de uma mesma linha compartilham exatamente o mesmo conjunto de `lineKey` membros (cada linha tem um segmento em cada coluna), o que colapsa colunas fisicamente distintas na mesma identidade e impede a formação de envelopes pareados por coluna.
- **H3c, sob sua definição pré-registrada literal, nunca produziu o resultado sintético 20/20 nem os resultados reais reportados no commit de avaliação (`ea56cf9`).** É classificada como **reprovada / mal especificada** sob a definição congelada em §6.4 — nunca "aprovada com correção de bug".

### H3c-r1 (revisão pós-pré-registro, criada depois da observação da falha)

- Identidade de agrupamento: `alignmentType + ":" + segmentKeys-dos-membros-ordenados-e-concatenados` — alteração feita **depois** de observar que H3c (definição original) falhava em P1/P5-P8, nunca antes.
- É esta variante, e não H3c conforme §6.4 original, que passou 20/20 casos sintéticos (`discovery-candidate-h3c-evaluation.test.ts`) e foi avaliada contra o manifesto real (`scripts/evaluate-h3c-real-manifest.ts`, commit `ea56cf9`): 25 acerto, 0 falso_positivo, 174 falso_negativo, 470 evidência_insuficiente, 1 incerto.
- **Estes resultados são exploratórios pós-emenda de H3c-r1 — nunca a confirmação de uma candidata integralmente pré-registrada.** O manifesto real (§13-§14) permanece válido como conjunto de rótulos congelado antes de qualquer execução de candidata — o que perdeu a condição integralmente pré-registrada foi a definição da CANDIDATA (H3c → H3c-r1), nunca os rótulos reais.
- Ver `EPIC_21_SPRINT_4B3A1_H3C_EVALUATION_REPORT.md` para o relatório completo desta distinção e suas implicações no veredito.

### Texto original desta seção (classificação superada pela correção acima — preservado, nunca apagado)
