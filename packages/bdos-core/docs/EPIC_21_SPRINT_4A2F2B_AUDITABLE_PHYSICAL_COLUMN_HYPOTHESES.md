...
**Prova integrada do cenário arquitetural real** (auditoria pós-revisão, §2): teste de pipeline com geometria sintética, reconstrução estrutural real, detecção de regiões real e reconstrução de hipóteses real (`reconstruct-budget-document-physical-column-hypotheses.test.ts`) em que uma coluna A recorre (só pela borda esquerda) em seis linhas físicas — antes, dentro e depois de qualquer região — e uma coluna B (largura fixa, três tipos de alinhamento) recorre apenas nas três linhas internas. A f.2a forma uma região de exatamente três linhas usando ambos os alinhamentos como suporte; a f.2b projeta o alinhamento de A para dentro da região (retendo apenas as três linhas internas, nunca as três externas) e produz duas hipóteses válidas — uma para A, uma para B —, ambas as evidências preservadas (`contributingAlignmentKeys` aponta para os dois `alignmentKey` originais), nenhum segmento externo atribuído à região (seis segmentos no total, todos incluídos), conservação e determinismo confirmados em duas execuções independentes. Este teste fabrica somente a linha física de entrada (posição, texto e geometria dos segmentos) — nunca fabrica manualmente os resultados de reconstrução estrutural, detecção de regiões ou hipóteses, que são sempre produzidos pelos três orquestradores reais e encadeados naturalmente. A prova completa da cadeia iniciada em bytes de PDF sintético, passando pelo leitor `pdfjs` real, é um teste separado (`infrastructure/budget-document-location/pdfjs/reconstruct-budget-document-physical-column-hypotheses.real-pdf-chain.test.ts`, ver parágrafo anterior); o cenário específico de alinhamento que ultrapassa a região é comprovado pela cadeia sintética geométrica descrita neste parágrafo, não por bytes de PDF, porque construir um documento sintético que produza duas colunas com abrangências de linha diferentes via texto/posição de PDF adicionaria complexidade de fixture sem valor probatório adicional sobre o que a cadeia real com PDF já comprova (§ parágrafo anterior).
