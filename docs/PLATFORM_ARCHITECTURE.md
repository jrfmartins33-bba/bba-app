...
| **Field Studio** | Execution Engine | `domain/execution-management`, `services/execution-management` (Epic 16, 16.1-16.8) | **Engine em produção** — domain model puro (`ExecutionWorkflow`/`ExecutionTask`, PRINCIPLE 006), Application Services (`createExecutionWorkflowFromActionPlan`, `materializeExecutionWorkflowFromRecommendation`), persistência real com RLS (`execution_workflows`/`execution_tasks`/`execution_task_evidence_references`/`execution_task_status_history`), repository/API mínima (`apps/web/lib/bdos/execution-repository.ts`) e handoff real a partir do Decision Copilot (`approveRecommendationId`, ver `packages/bdos-core/docs/COPILOT_WORKFLOW_HANDOFF.md`). **Field Studio (a tela de campo) continua Planejado** — nenhuma UI própria existe (sem diário de obra, checklist, apontamento manual); o único consumidor real hoje é o Decision Copilot, não uma interface de Field Studio. Distinção deliberada: Engine (tecnologia) ≠ Studio (produto), ver seção 14 para o status do Studio em si. |
